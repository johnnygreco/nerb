...
def make_synthetic_bank(
    *,
    name_count: int,
    patterns_per_name: int,
    entity_count: int = 16,
    literal_ratio: float = 1.0,
    bank_id: str = "synthetic_scale_bank",
) -> dict[str, Any]:
    """Create a deterministic JSON bank for local scale benchmarking."""
    _ensure_positive_int(name_count, "name_count")
    _ensure_positive_int(patterns_per_name, "patterns_per_name")
    _ensure_positive_int(entity_count, "entity_count")
    if not isinstance(literal_ratio, (int, float)) or isinstance(literal_ratio, bool) or not 0 <= literal_ratio <= 1:
        raise ExtractionError("Synthetic bank literal_ratio must be a number between 0 and 1.")

    active_entity_count = min(entity_count, name_count)
    entities = {
        f"entity_{entity_index:03d}": {
            "description": "Synthetic scale benchmark entity.",
            "status": "active",
            "regex_flags": [],
            "names": {},
            "metadata": {},
        }
        for entity_index in range(active_entity_count)
    }

    total_patterns = name_count * patterns_per_name
    literal_pattern_count = round(total_patterns * float(literal_ratio))
    pattern_ordinal = 0

    for name_index in range(name_count):
        entity_id = f"entity_{name_index % active_entity_count:03d}"
        name_id = f"name_{name_index:06d}"
        patterns: dict[str, Any] = {}
        for pattern_offset in range(patterns_per_name):
            is_literal = _synthetic_literal_slot(pattern_ordinal, total_patterns, literal_pattern_count)
            pattern_kind = "literal" if is_literal else "regex"
            pattern_id = f"{'lit' if pattern_kind == 'literal' else 'rx'}_{pattern_offset:03d}"
            value = f"nerb_scale_{entity_id}_{name_id}_alias_{pattern_offset:03d}"
            patterns[pattern_id] = (
                _synthetic_literal_pattern(value) if pattern_kind == "literal" else _synthetic_regex_pattern(value)
            )
            pattern_ordinal += 1

        names = entities[entity_id]["names"]
        if not isinstance(names, dict):
            raise TypeError("Synthetic entity names must be a mutable mapping.")
        names[name_id] = {
            "canonical": f"Synthetic Name {name_index:06d}",
            "description": "Synthetic scale benchmark name.",
            "status": "active",
            "patterns": patterns,
            "metadata": {},
        }
    ...
    return {
        ...
        "id": bank_id,
        "name": "Synthetic Scale Bank",
        "description": "Deterministic bank generated by nerb.benchmarks.make_synthetic_bank.",
        "version": "2026.06.03",
        "status": "active",
        ...
        "entities": entities,
        "metadata": {
            "generator": "nerb.benchmarks.make_synthetic_bank",
            "name_count": name_count,
            "patterns_per_name": patterns_per_name,
            "entity_count": active_entity_count,
    ...
    *,
    ...
    return {
        ...
        "engine": {
            ...
            "matchers": _matcher_profiles(compiled),
        },
        "options": {
...
def _synthetic_literal_pattern(value: str) -> dict[str, Any]:
    return {
        "kind": "literal",
        "value": value,
        "description": "Synthetic exact literal benchmark pattern.",
        "status": "active",
        "priority": 100,
        "case_sensitive": True,
...
def _synthetic_literal_slot(pattern_ordinal: int, total_patterns: int, literal_pattern_count: int) -> bool:
    previous_literals = (pattern_ordinal * literal_pattern_count) // total_patterns
    current_literals = ((pattern_ordinal + 1) * literal_pattern_count) // total_patterns
    return current_literals > previous_literals


def _synthetic_regex_pattern(value: str) -> dict[str, Any]:
    return {
        "kind": "regex",
        "value": re.escape(value),
        "description": "Synthetic regex benchmark pattern.",
        "status": "active",
        "priority": 50,
        "regex_flags": [],
        "metadata": {"benchmark_text": value},
    }
