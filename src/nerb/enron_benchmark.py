...
def build_enron_entity_bank(
    ...
    *,
    ...
    """Historical v1 fixture builder; current Enron work uses the v2 development workflow."""

    addresses = _historical_top_items(address_counts, max_items=max_addresses, min_count=min_address_count)
    domains = _historical_top_items(domain_counts, max_items=max_domains, min_count=min_domain_count)
    entities: dict[str, Any] = {}
    if addresses:
        entities["email_address"] = _historical_literal_entity(
            "Email addresses mined from training-set message headers and bodies.",
            addresses,
            pattern_description="Exact email address literal.",
        )
    if domains:
        entities["email_domain"] = _historical_literal_entity(
    ...
    return {
        ...
        "description": "Historical deterministic v1 fixture bank.",
        "version": "2026.06.09",
        "status": "active",
        ...
        "metadata": {
            ...
            "historical_nonpromotable": True,
        },
    }


def _historical_literal_entity(description: str, values: Sequence[str], *, pattern_description: str) -> dict[str, Any]:
    names: dict[str, Any] = {}
    for priority, value in enumerate(values):
        name_id = "v_" + hashlib.sha256(value.encode("utf-8")).hexdigest()[:16]
        names[name_id] = {
            "canonical": value,
            "description": "Corpus-mined historical literal.",
            "status": "active",
            "patterns": {
                "primary": {
                    "kind": "literal",
...
def _historical_top_items(counts: Counter[str], *, max_items: int, min_count: int) -> list[str]:
